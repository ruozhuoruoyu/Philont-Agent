...
One idea, applied everywhere: **an agent's reliability comes from the loop it runs, not the prompt it reads.** A prompt instruction is a suggestion the model can ignore; a loop is code it cannot. Five consequences:
...
- **It's a being, not a task runner.** Intrinsic drives (curiosity / pursuit / commitment) generate its own goals at idle time; it keeps an evidence-backed self-model of how it actually behaves; and its constitution evolves only through amendment proposals **you ratify** — red lines can never be amended. All of it visible: send `/autonomy` in chat or open the dashboard. → [Selfhood Engineering](https://github.com/ruozhuoruoyu/Philont-Agent/wiki/Selfhood-Engineering) · [design doc](docs/design/selfhood_closure.md)
- **It learns from failure — and repairs itself.** Skills and service recipes carry maturity grades and decay, **re-verified every time they're reused** — one that stops working is caught by its own check, then, at idle time, **diagnosed from its own failed runs in the execution ledger and rewritten**; the prior version is kept in a revision history so the fix has to re-earn trust, and a recipe that resists three repairs is retired. Personality traits are tuned from its own drive-effectiveness track record, not hardcoded constants, and routing confidence for what to do next rises and falls with real outcomes. → [Autonomy & Self-Learning](https://github.com/ruozhuoruoyu/Philont-Agent/wiki/Autonomy-and-Self-Learning)
