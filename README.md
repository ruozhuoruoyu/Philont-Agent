...
[![Built with loop engineering](https://img.shields.io/badge/built%20with-loop%20engineering-0ea5e9.svg)](#loop-engineering-the-rigor-lives-in-the-loop-not-the-prompt)
...
## Loop engineering: the rigor lives in the loop, not the prompt

**Loop engineering** is the 2026 shift in how serious agents are built: the reliability of an agent comes not from a cleverer system prompt but from **the loop it runs** — what triggers a turn, what bounds it, when it stops, what it reports, and where its progress is anchored. A prompt instruction is a suggestion the model can ignore; a loop is code it cannot. Philont is built end-to-end on that principle, and its loop carries a deliberate **spine** (the design is in [`docs/design/motivation_loop_architecture.md`](docs/design/motivation_loop_architecture.md)):
...
- **A goal-loop runtime with an explicit contract.** A committed goal runs on a loop with a written **Loop Contract** — *trigger · scope · budget · stop · report* — that decides each tick: keep going, stop, escalate, or **switch approach** when a path stops paying off (a `scoreTrajectory` metric reads the multi-turn *trajectory*, not one turn in isolation). It reports progress proactively, and pauses to ask before it spends past its round/token budget rather than grinding silently.
- **Intrinsic drives that generate goals.** Curiosity / pursuit / commitment drives don't just fire one-shot lookups — a sustained, high-stake, open theme is **promoted to a committed goal-loop**, tuned by the agent's own personality traits (好胜 / 好奇 / 尽责 → how hard it tries before declaring stuck, how loudly it reports).
