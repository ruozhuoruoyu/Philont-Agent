...
| `PHILONT_DRAFT_VALIDATION` | on | On idle, tests one never-used draft skill against a matching historical verifier failure. Only an actual tool run changes usage/maturity; declined or unsafe rewrites enter a persistent cooldown. Set `0`/`off` to disable. |
