...
test('a cleanup command anchors a fresh episode', () => {
  ...
  assert.match(cleanupBlock, /const episodeStartedAt = Date\.now\(\);[\s\S]*episodeAnchorTs\.set\(sessionId, episodeStartedAt\)/,
...
test('the anchor is set before the viability gate reads it', () => {
  ...
  const anchorAt = src.indexOf('episodeAnchorTs.set(sessionId, episodeStartedAt)');
