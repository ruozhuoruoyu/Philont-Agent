...
function renderSkillRepairPrompt(init: Initiative, repair: SkillRepairContext): string {
  ...
  return [
    ...
    '- **The fix is a REUSABLE recipe, not a one-off patch for this machine.** It will run again, on',
    '  other machines, under a permission layer. Therefore:',
    '    • Do NOT assume an OS, shell, or package manager (no apt-get/brew/choco/yum, no sudo, no',
    '      Windows-vs-POSIX-specific paths) unless the recipe already declared that environment.',
    '    • When the failure is a MISSING external dependency (a binary/tool not found), the correct fix',
    '      is to DETECT it and REPORT it clearly (e.g. `command -v X` → if absent, stop with a message',
    '      naming X) — NOT to install it, upgrade it, or otherwise mutate the system. Installing software,',
    '      changing global config, or any privileged/side-effecting remediation is out of bounds: a recipe',
    '      must not silently escalate what it does to the host every time it is reused.',
    '    • Prefer making the recipe FAIL CLEARLY and EARLY over making it try to self-heal the environment.',
    '- If the real fix would require stepping outside these bounds (e.g. the tool genuinely must be',
    '  installed), that is NOT a repairable recipe — OMIT skillRevision and say so in the summary.',
